...
export default function NewTaskForm() {
  const { equipmentId } = useLocalSearchParams<{ equipmentId: string }>(); const { colors } = useTheme();
  const { addTask } = useData(); const router = useRouter(); const insets = useSafeAreaInsets();
  const [name, setName] = useState(""); const [iv, setIv] = useState("3"); const [iu, setIu] = useState<IntervalUnit>("months"); const [notes, setNotes] = useState("");
  const save = useCallback(() => { if (!name.trim() || !equipmentId) return;
    addTask({ equipmentId, name: name.trim(), intervalValue: parseInt(iv, 10) || 1, intervalUnit: iu, lastCompletedDate: null, notes: notes.trim() });
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); router.back();
  }, [name, iv, iu, notes, equipmentId, addTask, router]);
  return (
    <View style={[s.c, { backgroundColor: colors.background }]}>
      <View style={[s.h, { paddingTop: insets.top + 8 }]}><TouchableOpacity onPress={() => router.back()} activeOpacity={0.6}><X size={24} color={colors.text} /></TouchableOpacity><Text style={[s.ht, { color: colors.text }]}>Add Task</Text><View style={{ width: 24 }} /></View>
