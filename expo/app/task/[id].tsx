...
export default function TaskView() {
  ...
  const { tasks, deleteTask, addCompletionLog, getLogsForTask, deleteCompletionLog } = useData();
  const router = useRouter(); const insets = useSafeAreaInsets();
  const task = tasks.find(t => t.id === id); const allLogs = useMemo(() => id ? getLogsForTask(id) : [], [id, getLogsForTask]);
  const historyLogs = useMemo(() => allLogs.length > 1 ? allLogs.slice(1) : [], [allLogs]);
  const status = task ? getTaskStatus(task) : "not_started"; const si = SI[status];
  const [modal, setModal] = useState(false); const [notes, setNotes] = useState("");
  ...
  const handleUndo = useCallback(() => {
    ...
    console.log('[TaskView] Undid completion:', undoLogId);
  ...
  const del = useCallback(() => { if (!task) return; Alert.alert("Delete this task?", "All history will be removed.", [
    { text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive", onPress: () => { deleteTask(task.id); router.back(); } }]);
  }, [task, deleteTask, router]);
  const delLog = useCallback((lid: string) => { Alert.alert("Delete entry?", "", [{ text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive", onPress: () => deleteCompletionLog(lid) }]); }, [deleteCompletionLog]);
  ...
  return (
    <View style={[st.c, { backgroundColor: colors.background }]}>
      <View style={[st.h, { paddingTop: insets.top + 8 }]}>
        ...
        <View style={st.ha}><TouchableOpacity onPress={() => router.push(`/task/edit/${task.id}`)} activeOpacity={0.6}><Pencil size={20} color={colors.textSecondary} /></TouchableOpacity>
      ...
      <View style={[st.bb, { paddingBottom: insets.bottom + 12, backgroundColor: colors.background }]}>
        ...
        <TouchableOpacity testID="complete" style={[st.cb, { backgroundColor: status === "overdue" ? "#EF4444" : colors.accent }]} onPress={() => setModal(true)} activeOpacity={0.8}>
          <CheckCircle size={22} color="#FFF" /><Text style={st.ct}>{status === "overdue" ? "Mark Complete \u2014 Overdue" : "Mark Complete"}</Text></TouchableOpacity>
      </View>
      <Modal visible={modal} transparent animationType="fade"><View style={st.ov}><View style={[st.md, { backgroundColor: colors.card }]}>
        <Text style={[st.mt, { color: colors.text }]}>Log Completion</Text><Text style={[st.ml, { color: colors.textSecondary }]}>Notes (optional)</Text>
        ...
        <View style={st.ma}><TouchableOpacity style={[st.mb, { borderColor: colors.border }]} onPress={() => setModal(false)}><Text style={[st.mbt, { color: colors.text }]}>Cancel</Text></TouchableOpacity>
          <TouchableOpacity style={[st.mb, { backgroundColor: colors.accent }]} onPress={complete}><Text style={[st.mbt, { color: "#FFF" }]}>Save</Text></TouchableOpacity></View>
      </View></View></Modal>
    </View>
  );
}
const st = StyleSheet.create({
  ...
  back: { padding: 4 }, ht: { fontSize: 20, fontWeight: "700" as const, flex: 1 }, ha: { flexDirection: "row", gap: 16 },
  sc: { flex: 1 }, si: { paddingHorizontal: 20 },
  card: { borderRadius: 14, borderWidth: 1, padding: 20, marginBottom: 24, gap: 8 },
  badge: { alignSelf: "flex-start", paddingHorizontal: 12, paddingVertical: 5, borderRadius: 8 }, badgeT: { color: "#FFF", fontSize: 13, fontWeight: "700" as const },
  ...
  md: { borderRadius: 16, padding: 24 }, mt: { fontSize: 20, fontWeight: "700" as const, marginBottom: 16 },
