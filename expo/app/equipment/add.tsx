      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={s.f} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.g}>{EQ_TYPES.map(t => { const a = type === t; return (
            <TouchableOpacity key={t} style={[s.tc, { backgroundColor: a ? colors.accent+"14" : colors.card, borderColor: a ? colors.accent : colors.border }]} onPress={() => { setType(t); if (t !== "custom") setEmoji(equipmentEmojis[t]); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }} activeOpacity={0.7}>
